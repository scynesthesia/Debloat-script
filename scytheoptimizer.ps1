...
Import-Module (Join-Path $moduleRoot "ui.psm1") -Force
Import-Module (Join-Path $moduleRoot "privacy.psm1") -Force
Import-Module (Join-Path $moduleRoot "debloat.psm1") -Force
Import-Module (Join-Path $moduleRoot "performance.psm1") -Force
Import-Module (Join-Path $moduleRoot "aggressive.psm1") -Force
...
if ($hardwareProfile.IsLaptop) { Write-Host "Laptop" -ForegroundColor Yellow } else { Write-Host "Desktop" -ForegroundColor Green }
...
do {
    ...
    switch ($choice) {
        '1' { Run-SOCPreset -HardwareProfile $hardwareProfile -StatusRef ([ref]$status) }
